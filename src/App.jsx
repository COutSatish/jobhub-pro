      ...
      <main className="flex-1 ml-64 relative z-10 p-8 lg:p-12 xl:p-16">
        ...
        {hasQuery && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-5xl space-y-12 pb-20">
            ...
            <section>
              <h3 className="text-lg font-bold text-white font-manrope mb-6 flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-[#ff007f]/10 flex items-center justify-center"><Network className="w-5 h-5 text-[#ff007f]" /></div>
                Careers & Jobs Subdomains
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {subdomains.map((item, index) => renderLinkCard(item, index, <Network className="w-5 h-5 txt-muted" />))}
              </div>
            </section>
